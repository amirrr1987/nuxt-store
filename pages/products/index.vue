...
<section class="py-12">
  <UContainer>
    <div class="grid md:grid-cols-2 lg:grid-cols-4 gap-4">
      <NuxtLink v-for="product in products" :to="{
        name: `product-id`,
        params:{
          id: product.id
        }
        ...
        <UCard >
          <template #header>
            <NuxtImg class="w-96 aspect-1 object-contain hover:scale-90  duration-300" :src="product.image" />
          </template>
          <UPopover mode="hover">
          ...
          <div>
            <UBadge color="primary" variant="subtle" :ui="{ rounded: 'rounded' }">{{ product.category}}</UBadge>
          </div>
        </UCard>
      </NuxtLink>
    </div>
